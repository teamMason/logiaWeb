<!-- Header -->

<div class="container">
    <div class="row ">
        <div class="col-lg-12 logo ">
            <img src="../../assets/img/portfolio/Logo1.svg" class=" text-center " alt="">
        </div>
    </div>
    <div class="row">
        <div class="intro-text text-center" style="padding-bottom: 200px; padding-top: 50px;">
            <div class="intro-lead-in">Bienvenido seas a nuestro portal</div>
            <div class="intro-lead-in"><h2>Masoneria Universal</h2></div>

            <div class=" flecha-abajo">
                <a href="#portfolio" title="" class="page-scroll flecha-abajo "><i class="fa fa-chevron-down fa-4x"></i></a>
            </div>
        </div>
    </div>
</div>

   
