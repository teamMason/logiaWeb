                            ...
                            <div class="container">
                                ...
                                <div class="col-md-8">
                                    {!! Form::model(Request::only(['edoCivil','path']),array('url' => '/miembros/solicitud',  'enctype' => 'multipart/form-data')) !!}
                                        <fielset><br>
                                            <input type="text" name = 'nombre' placeholder="Nombre" value="{{Input::old('nombre')}}"  class = "form-control " onblur="this.value=this.value.toUpperCase()"  required><br>
                                            <input type="text" name = 'apellido' placeholder="Apellido" value="{{Input::old('apellido')}}" class = "form-control" onblur="this.value=this.value.toUpperCase()"  required><br>

                                            <select name="id_taller" class ="form-control">
                                            @foreach($taller as $s)                                            
                                               <option value="{{$s->id_taller}}" selected="selected">{{$s->nombreTaller}}</option>
                                            @endforeach                                                                                                                                                      
                                            </select><br>  
                                            <select name="ciudad" class = "form-control">                                                 
                                            @foreach($taller as $d)  
                                               <option value="{{$d->ciudad}}" selected="selected">{{$d->ciudad}}</option>
                                             @endforeach                                                                                                     
                                            </select><br>
                                            <input type="text" name = 'profesion' placeholder="Profesión" value="{{Input::old('profesion')}}"  class = "form-control " onblur="this.value=this.value.toUpperCase()"  required><br>
                                            <select name="edoCivil"  value = "{{Input::old('edoCivil')}}" class = "form-control" size=5 required>
                                               <optgroup label="Estado Civil">
                                                 <option value="Casado">Casado</option>
                                                 <option value ="Soltero">Soltero</option>
                                                 <option value ="Union libre">Unión libre</option>
                                                 <option value ="Viudo">Viudo</option>
                                                 <option value="{{old('edoCivil')}}" selected>{{old('edoCivil')}}</option>
                                               </optgroup>
                                            </select><br>
                                            <input type="text" id="dinero" name="ingresoMen" placeholder ="Ingreso Mensual" class="form-control" value = "{{Input::old('ingresoMen')}}" required><br>
